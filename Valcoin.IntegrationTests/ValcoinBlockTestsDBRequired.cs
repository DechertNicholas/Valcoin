...
namespace Valcoin.IntegrationTests
{
    ...
    public class ValcoinBlockTestsDBRequired
    {
        ...
        public void VerifyReadWriteToDB()
        {
            ...
            input = new() // coinbase
            {
                PreviousTransactionId = "0x" + '0' * 64,
                PreviousOutputIndex = 0,
                UnlockerPublicKey = wallet.PublicKey,
                UnlockSignature = wallet.SignData(new UnlockSignatureStruct { BlockId = block.BlockId, PublicKey = wallet.PublicKey })
            };
            ...
            block.AddTx(tx);
            block.ComputeHash();
            fixture.DB.Add(block);

            var verify = fixture.DB.ValcoinBlocks.FirstOrDefault(b => b.BlockId == block.BlockId);
